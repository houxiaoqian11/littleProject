...
public class RefundHandler implements Handler {
	...
	public Map<String, Object> process(Map<String, Object> param) {
		logger.info("网银退款，平台传入参数为：" + param);
		...
		try {
			...
			returnMap.put(Constant.ERROR_MESSAGE, "网银退款异常:" + e.getMessage());
			returnMap.put(Constant.ERROR_SHOW_MESSAGE,ppc.getMessage("310030001"));
		}

		logger.info("网银退款，返回平台的结果为：" + returnMap);
		return returnMap;
	}
	...
	private Map<String, Object> fastPayBusinessProcess(Map<String, Object> param) {
		...
		try {
			...
			// 接口名称
			String service = ppc.getMessage("ucfpay.refund.service.name");
			...
			//后台通知地址
			String noticeUrl = ppc.getMessage("ucfpay.refund.noticeUrl");
			...
			logger.error("网银退款,组织上送报文异常:", e);
			returnMap.put(Constant.ACTION_RESULT, 1);
			returnMap.put(Constant.ERROR_CODE, "310030001");
			returnMap.put(Constant.ERROR_MESSAGE, "网银退款,组织上送报文异常" + e);
			returnMap.put(Constant.ERROR_SHOW_MESSAGE,ppc.getMessage("310030001"));
			return returnMap;
		}
		// 2、与银行通信
		...
		try {
			...
			logger.info("网银退款银行返回参数为：" + decryptData);
			...
			logger.error("网银退款,解析银行返回报文异常", e);
			returnMap.put(Constant.ACTION_RESULT, 2);
			returnMap.put(Constant.ERROR_CODE, "310020001");
			returnMap.put(Constant.ERROR_MESSAGE, "网银退款交易解析银行返回报文异常:" + e.getMessage());
