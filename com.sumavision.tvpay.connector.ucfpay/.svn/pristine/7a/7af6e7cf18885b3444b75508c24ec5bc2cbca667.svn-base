package com.sumavision.tvpay.connector.handler.ucfpay.bank;
...
public class WebBankTradeResultNoticeHandler implements Handler{
	...
	public Map<String, Object> process(Map<String, Object> params) {
		logger.info("b2c的异步通知解析handler，params="+params);
		...
		try{
			...
			logger.info("订单号:" + merchantNo + ",异步通知,银行返回交易成功");
			result.put("ACTION_RESULT", 0);
			...
			reqMap.put(Constant.CLIENT_ID, String.valueOf(new Date().getTime()) + merchantNo);
			...
			try {
        		...
        		logger.error("该交易交易类型[" + tradeCode + "]无法确认");
        		throw new ConnectorException("调用平台接口获取交易信息, 无法确认该交易交易类型TRADE_ID:" + merchantNo);
        	}
			...
			try {
				...
				if ((Integer) respMap.get(Constant.ACTION_RESULT) != 0) {
					...
					logger.info("B2C异步交易通知成功，平台处理正常");
					result.put("ACTION_RESULT", 0);// 成功
				}
			} catch (Exception e) {
				throw new ConnectorException("调用支付平台 支付异步确认接口异常!",e);
			}
		}catch(Exception e){
			logger.error("网银异步通知,报文解析异常", e);
		}
		return result;
	}
	...
	public String queryTradeInfo(String tradeId) {
		...
		try {
			...
			e.printStackTrace();
			throw new ConnectorException("310040404", "工商银行调用平台接口获取交易信息异常!", e);
		}
		return tradeCode;
	}
}