package com.sumavision.tvpay.connector.handler.ebank.fastpay.bank;
...
public class WebBankTradeResultNoticeHandler implements Handler{
	...
	public Map<String, Object> process(Map<String, Object> params) {
		...
		String tradeId = bankParam.get(Constant.TRADE_ID);
		String bankTradeId = bankParam.get(Constant.BANK_TRADE_ID);
		String sum = bankParam.get(Constant.SUM);
			
		String tradeCode = queryTradeInfo(tradeId);
		HashMap<String, Object> reqMap = new HashMap<String, Object>();
		reqMap.put(Constant.TRADE_ID, tradeId);
		...
		reqMap.put(Constant.BANK_CODE, "ebankfastpay");
		reqMap.put(Constant.BANK_TRADE_ID, bankTradeId);
		reqMap.put(Constant.ACTION_RESULT, 0);
		reqMap.put("SUM", new BigDecimal(sum));
		...
		try {
			...
			throw new ConnectorException("调用平台接口获取交易信息, 无法确认该交易交易类型TRADE_ID:" + tradeId);
		}
	...
	public String queryTradeInfo(String tradeId) {
		...
		try {
			...
			e.printStackTrace();
			throw new ConnectorException("310040404", "厦门民生调用平台接口获取交易信息异常!", e);
		}
		return tradeCode;
	}
